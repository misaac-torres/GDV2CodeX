...
vi.mock('../../hooks/useProjects', () => ({
  useProjects: () => ({ create: vi.fn().mockResolvedValue({}) })
}));
...
describe('AltaProyectos', () => {
  it('crea un proyecto con dependencias', async () => {
    ...
    await user.type(screen.getByLabelText('Descripción', { selector: 'textarea' }), 'Descripción de prueba');
    ...
    const depDesc = screen.getByLabelText('Descripción de dependencia');
    await user.type(depDesc, 'Dependencia de canales');

    await user.click(screen.getByTestId('guardar-proyecto'));
    expect(screen.getByLabelText('Nombre', { selector: 'input' })).toBeInTheDocument();
  }, 10000);
});
